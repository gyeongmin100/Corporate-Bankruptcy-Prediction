...
라벨을 부여한 81개 기업(결측 6개 제외 75개)의 **2023년 재무제표**를 모델에 입력해 1년 뒤(24년) 결과를 예측할 수 있는지 검증. 모델은 이 기업들의 **24년 데이터로 학습**되었으므로 순수 외부 검증이 아니라, 동일 기업의 1년 전 시점에서 조기 경보가 가능한지 측정하는 **시계열 백테스트**임 (진정한 out-of-sample 검증은 위 17개 hold-out). `test75.csv`에 데이터 공개.
...
**FP — 오탐 정상 2개** (실제 0인데 1로 예측). 재무비율상 실제로 부실 신호가 존재:

| 회사 | 부도확률 | 주요 원인 (SHAP) |
|---|---|---|
| 엠로 | 0.949 | 순운전자본비율 -35, 유동비율 46, 부채비중 68.5 |
| 인트로메딕 | 0.902 | 유동비율 98.5, 부채비중 56.3, 자기자본증가율 -47 |

→ 재무비율은 부도 판단의 기초 토대를 제공하지만, 급악화 이전 시점·비재무적 사유·감사의견과의 괴리 등이 존재하므로 **재무제표 주석·감사보고서 등을 함께 활용해야 입체적인 분석이 가능**함을 확인
...
```
├── README.md
├── test75.csv                  # 23년 백테스트셋 (부도 31 + 정상 44)
