...
| 외부 검증 (23년 변형의견 기업 36개) | **Recall 75%** (27/36 탐지) |
...
### 1. EDA 및 가설 검정 (`notebooks/data_exploration.ipynb`)

- 라벨 0/1 그룹 간 변수 분포 차이를 violin/swarm plot으로 시각화 (IQR 기준 이상치 제외 후)
- 유의성 검증: 표본 수가 적고(51 vs 30) 정규성 불만족 → 등분산성(Levene) 검정 후 t-test 및 **Mann-Whitney U test** (비모수 검정)로 유의미한 변수 도출
- 상관계수 |r| ≥ 0.7 변수쌍 확인 후 중복 변수 제거
- baseline으로 **로지스틱 회귀** (StratifiedKFold 5-fold OOF + threshold 튜닝)를 먼저 시도한 뒤 본 모델로 발전
...
- **유동비율**의 영향력이 가장 큼 (낮을수록 부도 방향) — 단, 라벨 0 선정 기준에 유동비율이 포함되어 효과가 과대평가될 수 있음 (한계 참고)
- 자기자본증가율이 낮을수록 / 매입채무 대 재고자산비율, 타인자본구성비율, 차입금의존도가 높을수록 부도 방향

### 6. 오분류 분석

FN(실제 1인데 0으로 예측) 9개 기업을 SHAP로 개별 분석:
...
│   └── 학습데이터.csv          # EDA·가설검정용 초기 데이터
├── docs/
│   └── presentation.pdf        # 최종 발표 자료
├── images/                     # 결과 이미지 (노트북 출력)
└── notebooks/
    ├── data_exploration.ipynb  # EDA·가설검정·변수선별·로지스틱 baseline
