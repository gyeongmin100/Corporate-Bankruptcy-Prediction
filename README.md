...
| 외부 검증 - 균형 (23년 재무, 75개: 부도 31 + 정상 44) | **AUC 0.952** · precision 0.926 / recall 0.806 / FPR 0.045 |
| 외부 검증 - 부도만 (23년 변형의견 기업 36개) | **Recall 77.8%** (28/36 탐지) |
...
**23년 외부 검증 - 균형 백테스트 (부도 31개 + 정상 44개, 총 75개)**

라벨 1(부도, 31개)과 라벨 0(정상, 44개) 기업 각각의 **2023년 재무제표**를 모델에 입력해 24년 결과를 맞히는지 검증. `test75.csv`에 데이터 공개.
...
- AUC 0.952 · AP 0.948 · FPR(오탐률) 4.5% (44개 중 2개만 오탐)
- 정상기업까지 포함한 균형 검증에서도 높은 정밀도(92.6%)와 낮은 오탐률을 확인 → 아래 36개 검증(부도 기업만 포함)의 recall 편중 문제를 보완

**23년 외부 검증 - 부도 기업만 (변형의견 + 계속기업 존속 의문 기업 36개, 라벨 전부 1)**
...
- 단, 이 36개는 라벨이 전부 1이라 recall만 계산 가능 → 정밀도·오탐률은 위 75개 균형 백테스트로 보완
...
- hold-out test가 17개로 작아 지표의 신뢰구간이 넓음 → 23년 75개 균형 백테스트로 보완

## 저장소 구조

```
├── README.md
├── test75.csv                  # 23년 균형 백테스트셋 (부도 31 + 정상 44)
