...
어떤 재무비율이 부도 판단에 가장 큰 영향을 미치는지 SHAP(TreeExplainer)으로 분석:

![SHAP summary plot](images/shap_summary.png)
![SHAP feature importance](images/shap_importance.png)

- **유동비율**의 영향력이 압도적 (낮을수록 부도 방향) — 단, 라벨 0 선정 기준에 유동비율이 포함되어 효과가 과대평가될 수 있음 (한계 참고)
- 자기자본증가율, 매출액총이익률이 낮을수록 / 타인자본구성비율, 차입금의존도가 높을수록 부도 방향
...
├── images/                     # 결과 이미지 (노트북 출력 추출)
└── notebooks/
    └── BA_project.ipynb        # 전체 파이프라인 (train/test 분리 버전)
```

노트북은 [Google Colab에서 바로 열기](https://colab.research.google.com/drive/10IBjSchHlI2xqlR71X7JyXrAEelGX3Sm?usp=sharing)로 실행 흐름을 확인할 수 있습니다.
