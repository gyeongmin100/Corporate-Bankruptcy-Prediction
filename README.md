...
| 23년 백테스트 (라벨 75개: 부도 31 + 정상 44) | **AUC 0.952** · precision 0.926 / recall 0.806 / FPR 0.045 |
...
**23년 백테스트 (라벨 부여 기업 75개: 부도 31 + 정상 44)**

라벨을 부여한 81개 기업(결측 6개 제외 75개)의 **2023년 재무제표**를 모델에 입력해 1년 뒤(24년) 결과를 예측할 수 있는지 검증. 모델은 이 기업들의 **24년 데이터로 학습**되었으므로 순수 외부 검증이 아니라, 동일 기업의 1년 전 시점에서 조기 경보가 가능한지 측정하는 **시계열 백테스트**임 (진정한 out-of-sample 검증은 위 17개 hold-out).

```
[[42  2]
 [ 6 25]]
              precision    recall  f1-score   support
           0      0.875     0.955     0.913        44
           1      0.926     0.806     0.862        31
    accuracy                          0.893        75
```

- AUC 0.952 · AP 0.948 · FPR(오탐률) 4.5% (정상 44개 중 2개만 오탐)
- 부도 시그널은 한순간이 아니라 점진적으로 누적됨 → 24년 부실 기업의 **23년 재무만으로도** 상당수가 높은 부도 확률로 예측되어, 1년 전 조기 경보 가능성을 확인
- 부도 기업(recall 0.806)뿐 아니라 정상 기업까지 포함한 균형 구성으로 **정밀도(0.926)·오탐률(0.045)** 까지 함께 측정
...
75개 백테스트의 오분류 8건(FN 6 + FP 2)을 분석.

**FN — 놓친 부도 6개** (실제 1인데 0으로 예측). SHAP로 개별 분석:

![오분류(FN) 기업 SHAP summary](images/shap_fn_summary.png)

FN 6개는 23년 재무 기준 위험 신호가 약해(부채비율 낮고 유동비율 양호) 모델이 낮은 확률을 부여. 사유를 나누면:

- **재무 급악화형 4개** (이오플로우·코스나인·드래곤플라이·비에프랩스): 23→24년 유동비율·순운전자본이 급락(예: 이오플로우 유동비율 338→28, 순운전자본비율 22→-41). 23년 시점엔 재무가 아직 정상이라 1년 전 예측이 불가능
- **비재무 사유형 2개** (아이에이치큐·테라사이언스): 24년에도 유동비율이 정상(143·290)이나, **감사범위 제한(감사증거 미입수)** 으로 의견거절 → 재무비율과 무관한 사유

**FP — 오탐 정상 2개** (실제 0인데 1로 예측). 23년 재무가 실제로 부실 상태였다가 24년에 급개선된 **부실→회복 전환** 기업:

| 회사 | 부도확률 | 23년 재무 (SHAP상 부실 신호) | 24년 회복 |
|---|---|---|---|
| 엠로 | 0.949 | 순운전자본 -35, 유동비율 46, 부채비중 68.5 | 유동비율 401, 순운전자본 +27 |
| 인트로메딕 | 0.902 | 순운전자본 -0.6, 유동비율 98.5, 부채비중 56.3 | 유동비율 233, 순운전자본 +43 |

23년 시점 재무만으로는 부실기업과 구분 불가 → 모델의 오탐이라기보다 **부실→회복 전환의 조기 식별 한계**

→ FN(재무 악화 직전)과 FP(재무 회복 직전) 모두 **전환기의 재무제표는 판단이 어렵다**는 공통 구조. 재무비율은 부도 판단의 기초 토대를 제공하지만, 비재무적 사유·전환기 상태 등이 존재하므로 **재무제표 주석·감사보고서 등을 함께 활용해야 입체적인 분석이 가능**함을 확인

## 한계

- 유동비율을 기준으로 라벨 0을 선정하여 해당 변수의 효과가 과대평가될 수 있음
- 재무비율은 양방향 해석이 가능함 (같은 비율도 맥락에 따라 위험/정상 신호로 읽힐 수 있음)
- hold-out test가 17개로 작아 지표의 신뢰구간이 넓음 → 23년 75개 백테스트로 보완
- 23년 백테스트 75개는 모두 24년 라벨을 부여한 학습 기업이므로 순수 외부 검증이 아님 (동일 기업의 1년 전 시점 조기경보 성능을 측정)
